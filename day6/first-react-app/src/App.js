...
function App() {
  const containerStyle = { display: 'grid', gap: '10px', gridTemplateColumns: '1fr 1fr 1fr 1fr' };
  ...
  return (
    <div className="App">
      <div style={containerStyle}>
        {productData.map(({ title, images, price, rating, stock }) => {
          return <ProductItem stock={stock} title={title} price={price} rating={rating} img={images[0]} />
        })}
      </div>
    </div>
  );
